...
include("VorticityController.jl")
...
function COM_handler(config::Dict{<:Any}, electrodes_order::Vector{<:Int})
    ...
    function SET(electrodes::Vector{<:Int}, coils::Vector{<:Number})
        #@show "sending: (electrodes)$electrodes || (coils)$coils"
        set_channels(elec_com, electrodes_order , electrodes) #collect(1:NUM_ELECTRODES)
        set_coil(coil_com, 1, 4, coils[1])
        #set_coil(coil_com, 2, 3, coils[2])
        set_coil(coil_com, 2, 2, coils[2])
        #set_coil(coil_com, 3, 2, coils[3])
...
function main()
    ...
    t = @elapsed while !quit
        ...
        if(start_sending && save)
            ...
            sample = sample + 1
            # check for overflow
            if(controller.k > controller_steps)
                quit = true
    ...
    JLD2.save(file_name * "-controller-" * "$(now()).jld", "X", controller.X[:, 1:controller.k], "phi", controller.Φ[:, 1:controller.k], "psi", controller.Ψ[:, 1:controller.k])
